...
class AdamLayer(NerveLayer):

    def __init__(self, node_num, pre_layer, linear_dict, activation_dict, input_layer=False, output_layer=False,
                 output=None, keep_prob=1.0):
        super(AdamLayer, self).__init__(node_num, pre_layer, linear_dict, activation_dict, input_layer,
                                             output_layer, output, keep_prob)
        self.beta1 = 0.9
        self.beta2 = 0.999
        self.epsilon = 1e-8
        self.v_dw = None
    ...
    def init_params(self):
        super(AdamLayer, self).init_params()
        self.v_dw = np.zeros(self.w.shape)
        self.v_db = np.zeros(self.b.shape)
        self.s_dw = np.zeros(self.w.shape)
        self.s_db = np.zeros(self.b.shape)

    def update_params_with_momentun(self, mean_rate=0.9, learning_rate=0.01):
        self.v_dw = mean_rate * self.v_dw + (1.0 - mean_rate) * self.dw
        self.v_db = mean_rate * self.v_db + (1.0 - mean_rate) * self.db
        self.w -= learning_rate * self.v_dw
        self.b -= learning_rate * self.v_db

    def update_params(self, learning_rate=0.01):
        self.t += 1
        self.v_dw = self.beta1 * self.v_dw + (1.0 - self.beta1) * self.dw
        self.v_db = self.beta1 * self.v_db + (1.0 - self.beta1) * self.db
        v_dw_correct = self.v_dw / (1.0 - np.power(self.beta1, self.t))
        v_db_correct = self.
